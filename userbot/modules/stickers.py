...
@register(outgoing=True, pattern="^\{trg}tachos".format(trg=trgg))
async def tachos(args):
    """ For .tachos command, kangs stickers or creates new ones. """
...
async def k(args):
    """ For .tachos command, kangs stickers or creates new ones silently. """
    user = await bot.get_me()
    if not user.username:
        user.username = user.first_name
    message = await args.get_reply_message()
...
CMD_HELP.update(
    {
        "stickers": ">`.tachos <emoji>[optional] <pack number>[optional]`"
